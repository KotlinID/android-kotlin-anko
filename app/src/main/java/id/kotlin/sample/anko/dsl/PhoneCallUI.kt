...
class PhoneCallUI : AnkoComponent<PhoneCallActivity> {

    override fun createView(ui: AnkoContext<PhoneCallActivity>) = with(ui) {
        coordinatorLayout {
            backgroundColor = ContextCompat.getColor(ctx, R.color.colorPrimary)

            relativeLayout {
                toolbar {
                    id = R.id.toolbar_phone
                ...
                button {
                    id = R.id.btn_call
                    text = ctx.resources.getString(R.string.btn_call)
                    textColor = ContextCompat.getColor(ctx, R.color.colorPrimaryDark)
                    textSize = 16f
